...
class Solution:
    def smallestNumber(self, num: str, t: int) -> str:
        ...
        def format(cnt, l):
            return '1'*(l-sum(cnt)) + "".join(str(x)*cnt[x-2] for x in range(2, 10))
        ...
        i = next((i for i in range(len(nums)) if nums[i] == 0), len(nums))
...
# Time:  O(nlogt)
# Space: O(logt)
# gcd-based candidate construction
class Solution2:
    def smallestNumber(self, num: str, t: int) -> str:
        def gcd(a, b):
            while b:
